...
interface AnalysisViewProps {
  ...
  setSelectedType: (type: EntryType | "縺吶∋縺ｦ") => void;
...
export function AnalysisView({ 
  ...
  const [viewGender, setViewGender] = useState<Gender>("繧ｪ繧ｹ");
  const itemRefs = useRef<Record<string, HTMLDivElement | null>>({});

  // 蜑榊屓縺ｮ螻暮幕謨ｰ繧剃ｿ晄戟縺励※縲∵眠隕上↓螻暮幕縺輔ｌ縺滓凾縺ｮ縺ｿ繧ｹ繧ｯ繝ｭ繝ｼ繝ｫ縺吶ｋ繧医≧縺ｫ縺吶ｋ
  ...
  useEffect(() => {
    if (expandedNames.length > prevExpandedCount.current) {
      ...
      if (element) {
        // 繧｢繝九Γ繝ｼ繧ｷ繝ｧ繝ｳ縺ｫ繧医ｋ繝ｬ繧､繧｢繧ｦ繝亥､画峩繧定・・縺励※驕・ｻｶ螳溯｡・
  ...
  // 邂｡逅・錐縺ｧ閾ｪ辟ｶ鬆・た繝ｼ繝医☆繧九Θ繝ｼ繝・ぅ繝ｪ繝・ぅ
  ...
  const groupedStats = useMemo(() => {
    ...
    entries.forEach((entry) => {
      const key = entry.scientificName || "譛ｪ險ｭ螳・;
      const mName = entry.managementName || "No Name";
      const currentGender = 
        entry.type === "謌占勠" ? entry.gender : 
        entry.type === "蟷ｼ陌ｫ" ? (entry.logs[0]?.gender || "荳肴・") : 
        "荳肴・";
      ...
      if (entry.type === "逕｣蜊ｵ繧ｻ繝・ヨ") {
        ...
        if (sEntry.eggCount !== undefined) {
          groups[key].spawnEggRecords.push({ val: sEntry.eggCount, mName, gender: "荳肴・", entryId: sEntry.id });
        }
        if (sEntry.larvaCount !== undefined) {
          groups[key].spawnLarvaRecords.push({ val: sEntry.larvaCount, mName, gender: "荳肴・", entryId: sEntry.id });
        }
        const total = (sEntry.eggCount || 0) + (sEntry.larvaCount || 0);
        groups[key].recoveryRecords.push({ val: total, mName, gender: "荳肴・", entryId: entry.id });
      }
      if (entry.type === "蟷ｼ陌ｫ") {
        ...
        if (entry.actualEmergenceDate) {
          // LarvaBeetle蝙九↓hatchDate縺悟性縺ｾ繧後※縺・↑縺・ｴ蜷医・繧ｨ繝ｩ繝ｼ繧貞屓驕ｿ
          const hatchDate = entry.hatchDate || entry.extractionDate || entry.createdAt; // 蟄ｵ蛹匁律 > 蜑ｲ蜃ｺ譌･ > 菴懈・譌･縺ｮ蜆ｪ蜈磯・ｽ・
        ...
        if (entry.size) {
          ...
          if (!isNaN(s)) {
            ...
            if (entry.linkedEntryId) {
              const linkedLarva = entries.find(e => e.id === entry.linkedEntryId && e.type === "蟷ｼ陌ｫ") as LarvaBeetle | undefined;
    ...
    return Object.values(groups).map((group) => ({
      ...
      parentAggregatedTotals: group.spawnSetEntries.reduce((acc, s) => {
        const ss = s as any;
        const name = ss.managementName || ss.japaneseName || "荳肴・";
  ...
  return (
    <div className="space-y-4">
            ...
            {expandedNames.includes(stat.scientificName) && (
              <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="px-5 pb-5 flex flex-col gap-4 border-t border-gray-50/50 pt-4">
                <div className="bg-white/60 border border-white/60 p-4 rounded-2xl">
                  <div className="text-[12px] font-black text-gray-400 uppercase mb-3 tracking-widest text-center">繧ｵ繧､繧ｺ險倬鹸 (MIN / MAX)</div>
                  ...
                  <button 
                    onClick={() => setSelectedAnalysis({ label: "菴馴㍾險域ｸｬ螻･豁ｴ", records: stat.weightRecords })}
                ...
                <AnalysisItem label="逕｣蜊ｵ譁ｹ豕・ value={stat.spawnSetEntries.length > 0 ? `${stat.spawnSetEntries.length}莉ｶ縺ｮ險倬鹸` : "-"} onClick={() => setSelectedSpawnTable(stat as any)} isLink />
                <AnalysisItem label="謌占勠繧ｵ繧､繧ｺ遽・峇" value={stat.sizeRange ? `${stat.sizeRange[0]}縲・{stat.sizeRange[1]}mm` : "-"} onClick={() => setSelectedAnalysis({ label: "謌占勠繧ｵ繧､繧ｺ蛻・ｸ・, records: stat.adultSizeRecords })} isLink />
                <AnalysisItem label="蝗槫庶蜷郁ｨ・(蜊ｵ+蟷ｼ陌ｫ)" value={stat.spawnResultRange.total ? `${stat.spawnResultRange.total[0]}縲・{stat.spawnResultRange.total[1]}蛟義 : "-"} onClick={() => setSelectedAnalysis({ label: "蝗槫庶蜷郁ｨ医ョ繝ｼ繧ｿ", records: stat.recoveryRecords })} isLink />
                <AnalysisItem label="莨醍悛譛滄俣遽・峇" value={stat.dormancyRange ? `${stat.dormancyRange[0]}縲・{stat.dormancyRange[1]}譌･` : "-"} onClick={() => setSelectedAnalysis({ label: "莨醍悛繝・・繧ｿ", records: stat.dormancyRecords })} isLink />
                <AnalysisItem label="蟇ｿ蜻ｽ遽・峇" value={stat.lifespanRange ? `${stat.lifespanRange[0]}縲・{stat.lifespanRange[1]}譌･` : "-"} onClick={() => setSelectedAnalysis({ label: "逕溷ｭ倥ョ繝ｼ繧ｿ", records: stat.lifespanRecords })} isLink />
                <AnalysisItem label="蟷ｼ陌ｫ譛滄俣遽・峇" value={stat.larvaRange ? `${stat.larvaRange[0]}縲・{stat.larvaRange[1]}譌･` : "-"} onClick={() => setSelectedAnalysis({ label: "蟷ｼ陌ｫ繝・・繧ｿ", records: stat.larvaRecords })} isLink />
                <div className="col-span-2">
                  <AnalysisItem label="隕ｪ蛻･蜷郁ｨ・ value={Object.keys(stat.parentAggregatedTotals).length > 0 ? `${Object.keys(stat.parentAggregatedTotals).length}隕ｪ縺ｮ蜷郁ｨ医ｒ遒ｺ隱港 : "-"} onClick={() => setSelectedAnalysis({ 
                    label: "隕ｪ蛻･蝗槫庶蜷育ｮ・, 
                    records: Object.entries(stat.parentAggregatedTotals).map(([name, val]) => ({ val: val.eggs + val.larvae, mName: name, gender: "荳肴・", entryId: "" }))
                  })} isLink />
                </div>
                <AnalysisItem label="謌占勠繧ｵ繧､繧ｺ vs 譛螟ｧ蟷ｼ陌ｫ菴馴㍾" value={stat.adultSizeVsMaxLarvaWeightRecords.length > 0 ? `${stat.adultSizeVsMaxLarvaWeightRecords.length}莉ｶ縺ｮ繝・・繧ｿ` : "-"} onClick={() => setSelectedAnalysis({
                  label: "謌占勠繧ｵ繧､繧ｺ vs 譛螟ｧ蟷ｼ陌ｫ菴馴㍾",
               ...
               {selectedAnalysis.scatterData ? (
                 <>
                   <div className="h-44 mb-4 bg-gray-50/50 rounded-2xl p-4 shrink-0">
                     <ResponsiveContainer width="100%" height="100%">
                       <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: -20 }}>
                         ...
                         <Scatter
                           data={selectedAnalysis.scatterData.filter(d => viewGender === "荳肴・" || d.gender === viewGender)}
                           fill={viewGender === "繧ｪ繧ｹ" ? "#E67E22" : viewGender === "繝｡繧ｹ" ? "#EC407A" : "#D35400"}
                   ...
                   <div className="flex-1 overflow-y-auto space-y-2 mb-4 pr-2 custom-scrollbar">
                     {selectedAnalysis.scatterData.filter(d => viewGender === "荳肴・" || d.gender === viewGender).length > 0 ? (
                       selectedAnalysis.scatterData.filter(d => viewGender === "荳肴・" || d.gender === viewGender).map((rec, i) => (
                       ...
                       <p className="text-center text-gray-400 py-10">繝・・繧ｿ縺後≠繧翫∪縺帙ｓ</p>
                     )}
                   </div>
                 </>
               ) : (
                   ...
                   <div className="h-44 mb-4 bg-gray-50/50 rounded-2xl p-4 shrink-0">
                     <ResponsiveContainer width="100%" height="100%">
                       <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: -20 }}>
                         ...
                         <Tooltip cursor={{ strokeDasharray: '3 3' }} content={({ active, payload }) => {
                             if (active && payload && payload.length) {
                               const unit = selectedAnalysis.label.includes("菴馴㍾") ? "g" :
                                            selectedAnalysis.label.includes("譛滄俣") || selectedAnalysis.label.includes("蟇ｿ蜻ｽ") ? "譌･" :
                                            selectedAnalysis.label.includes("繧ｵ繧､繧ｺ") ? "mm" :
                                            selectedAnalysis.label.includes("蜊ｵ") ? "蛟・ : "鬆ｭ";
                     ...
                     {(selectedAnalysis.records || []).filter(r => r.gender === viewGender).length > 0 ? (
                       ...
                       <div key={i} className="flex justify-between items-center p-4 bg-white shadow-sm rounded-xl font-black border border-gray-200">
                          ...
                          <span className="text-[#FF9800] text-lg leading-none">{rec.val}<span className="text-xs ml-0.5 font-bold">
                            {selectedAnalysis.label.includes("菴馴㍾") ? "g" :
                             selectedAnalysis.label.includes("譛滄俣") || selectedAnalysis.label.includes("蟇ｿ蜻ｽ") ? "譌･" :
                             selectedAnalysis.label.includes("繧ｵ繧､繧ｺ") ? "mm" :
                             selectedAnalysis.label.includes("蜊ｵ") ? "蛟・ : "鬆ｭ"}
                          </span></span>
      ...
      {/* 逕｣蜊ｵ繧ｻ繝・ヨ 繧ｨ繧ｯ繧ｻ繝ｫ鬚ｨ繝・・繝悶Ν繝｢繝ｼ繝繝ｫ */}
      <AnimatePresence>
        {selectedSpawnTable && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-white p-6 rounded-[32px] w-full max-w-lg max-h-[80vh] shadow-2xl relative z-10 flex flex-col overflow-hidden">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-black text-xl text-[#4A3F35]">逕｣蜊ｵ繧ｻ繝・ヨ險倬鹸: {selectedSpawnTable.japaneseName}</h3>
              ...
              >
                <PlusCircle size={16} /> 譁ｰ縺励＞繧ｻ繝・ヨ繧定ｦ区悽縺ｨ縺励※蜈･蜉・
              </button>
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <div className="font-bold text-gray-800">{s.managementName || "邂｡逅・錐縺ｪ縺・}</div>
                          <div className="text-[10px] text-gray-400 font-medium">
                            {s.setDate?.replace(/-/g, "/")} 縲・{s.setEndDate?.replace(/-/g, "/") || "邯咏ｶ壻ｸｭ"}
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] mb-3">
                        <div className="bg-white/80 p-2 rounded-xl border border-gray-50">
                          <span className="text-gray-400 block text-[9px] uppercase font-bold mb-0.5">菴ｿ逕ｨ繝槭ャ繝・/span>
                          <span className="font-bold text-gray-700 truncate block">{s.substrate || "-"}</span>
                        </div>
                        <div className="bg-white/80 p-2 rounded-xl border border-gray-50">
                          <span className="text-gray-400 block text-[9px] uppercase font-bold mb-0.5">貂ｩ蠎ｦ</span>
                          <span className="font-bold text-gray-700">{s.temperature ? `${s.temperature}邃チ : "-"}</span>
                        </div>
                        <div className="bg-white/80 p-2 rounded-xl border border-gray-50">
                          <span className="text-gray-400 block text-[9px] uppercase font-bold mb-0.5">隧ｰ蝨ｧ / 豌ｴ蛻・/span>
                          <span className="font-bold text-gray-700">{s.pressure || "-"} / {s.moisture}</span>
                        </div>
                        <div className="bg-white/80 p-2 rounded-xl border border-gray-50">
                          <span className="text-gray-400 block text-[9px] uppercase font-bold mb-0.5">蜷悟ｱ・/span>
                      ...
                      {s.memo && (
                        <div className="bg-white/50 p-2 rounded-xl border border-gray-50 mb-3">
                          <span className="text-gray-400 block text-[9px] uppercase font-bold mb-0.5">蛯呵・Γ繝｢</span>
                      ...
                      >
                        <PlusCircle size={14} /> 縺薙・蜀・ｮｹ繧定ｦ区悽縺ｨ縺励※譁ｰ隕冗匳骭ｲ
                      </button>
                    </div>
                  );
                })}
              </div>
              <p className="mt-4 text-[10px] text-gray-400 font-bold text-center italic">陦後ｒ繧ｿ繝・・縺励※隧ｳ邏ｰ繝・・繧ｿ縺ｸ繧ｸ繝｣繝ｳ繝・/p>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
      ...
      <section className="bg-white/80 backdrop-blur-md p-6 rounded-[24px] border border-white/60 shadow-sm mt-8">
        ...
        <div className="mb-4 p-4 bg-white/40 rounded-2xl border border-white/60 flex items-center justify-between">
          <div className="text-[10px] font-bold text-gray-600">
            {isPersisted ? "笨・豌ｸ邯壹せ繝医Ξ繝ｼ繧ｸ譛牙柑" : "笞・・蜑企勁縺輔ｌ繧句庄閭ｽ諤ｧ縺後≠繧翫∪縺・}
          </div>
          {!isPersisted && <button onClick={requestPersistence} className="text-[10px] bg-[#FF9800] text-white px-3 py-1 rounded-full font-bold">譛牙柑蛹・/button>}
        </div>
        <div className="grid grid-cols-2 gap-3">
          <button 
            onClick={handleSync} 
          ...
          >
            <Upload size={14} /> 
            {isSyncing ? "蜷梧悄荳ｭ..." : "GitHub縺ｸ繝・・繧ｿ繧貞酔譛・}
          </button>
          <button onClick={handleExport} className="flex items-center justify-center gap-2 bg-white/80 py-3 rounded-xl text-xs font-bold shadow-sm active:scale-95 transition-all"><Download size={14} /> 譖ｸ縺榊・縺・/button>
          <label className="flex items-center justify-center gap-2 bg-white/80 py-3 rounded-xl text-xs font-bold shadow-sm active:scale-95 transition-all cursor-pointer"><Upload size={14} /> 隱ｭ縺ｿ霎ｼ縺ｿ<input type="file" hidden onChange={handleImport} accept=".json" /></label>
        </div>
      </section>
    </div>
  );
